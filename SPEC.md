...
| **United Kingdom** | Skilled Worker, £41,700/yr floor or the going rate (higher of) | ILR after **5yr** continuous residence | 12mo after ILR (~6yr); **dual allowed** | **UK register of licensed sponsors (public)** — already ingested | Added slice 18 on supply evidence: 480 open / 47 firms, second only to SG. The register now speaks about the country the job is in, not just "sponsors somewhere" |
...
**UK sponsor register** is ingested directly. As of slice 18 the UK **is** a target country (row above), so a GB job at a registered employer is `registry_inferred` *about the country the job is actually in* — the strongest form that signal takes, and what the register was built to say. Its original secondary use survives unchanged: many multinationals appear on it, and presence there remains a company-level "sponsors somewhere" proxy for jobs in countries with no register of their own.
