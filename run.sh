...
# --- node/npm on PATH -------------------------------------------------------
# nvm's shell function is never loaded under a non-interactive shell, so `npm` can be
# missing even though node is installed. Shared with the Makefile's frontend targets so
# both resolve it the same way. Exits 127 if node really is absent.
